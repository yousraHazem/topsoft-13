class BudgetComponentsController < ApplicationController
	...
	def list
		...
		# calculates progress bar percentage and returns all the budget components of a certain budget item 
	...
	def create
		#authorized by: sarah ahmed id=22-1278
		# creates a budget component and then updates the spent and total attributes of its budget item
		@project_id = params[:project_id]
	    @item = params[:id]
	...
	def edit
		#authorized by: sarah ahmed id=22-1278
		#gets the budget component of the passed id and view its details in the edit form
		@item = params[:item_id]
		@project_id = params[:project_id]
	...
	def update
		#authorized by: sarah ahmed id=22-1278
		#updates the budget component with the new submotted values and then updates the spent and total attributes of the associated budget
		@item = params[:item_id]
		@project_id = params[:project_id]
		@budget_item= BudgetItem.find(@item)
		@component = BudgetComponent.find(params[:id])
	...
	def destroy
		#authorized by: sarah ahmed id=22-1278
		 #deletes the component with the passed id and then updates the spent and total attributes of the associated budget
	    @item = params[:item_id]
